...
def plan_batch(snapshots, regimes, inventories=None, limits=None, config=None):
    ...
    for snapshot in snapshots:
        ...
        regime=regimes.get(snapshot.symbol,Regime.UNKNOWN); inv=inventories.get(snapshot.symbol,0.0); eligibility=score_symbol(snapshot); risk=assess_risk(snapshot,regime,inv,0.0,limits)
        ...
        orders=_orders_for_quote(snapshot,quote,config,inv) if allowed else tuple(); gross+=sum(abs(o.price*o.quantity) for o in orders); net+=sum(_signed_delta(o) for o in orders)
        plans.append(SymbolPlan(snapshot.symbol,regime.value,allowed,risk.mode,asdict(quote),orders,tuple(blockers)))
    total=sum(len(p.orders) for p in plans); blocked=gross>config.max_gross_notional or abs(net)>config.max_net_notional or total>config.max_total_orders
    if blocked: plans=[SymbolPlan(p.symbol,p.regime,False,'pause',p.quote,tuple(),p.blockers+('global_exposure_or_order_limit',)) for p in plans]
